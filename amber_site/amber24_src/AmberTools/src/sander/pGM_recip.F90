...
# include "ew_parallel.h"
  ...
  allocate(theta1(0:dr_order,1:Bspline_order,num_ks), stat=ier)
  REQUIRE(ier==0)
  end if

  if (.not. allocated(theta2)) then
  allocate(theta2(0:dr_order,1:Bspline_order,num_ks), stat=ier)
  REQUIRE(ier==0)
  end if

  if (.not. allocated(theta3)) then
  allocate(theta3(0:dr_order,1:Bspline_order,num_ks), stat=ier)
  REQUIRE(ier==0)
  end if

  if (.not. allocated(init_grid_ind)) then
  allocate(init_grid_ind(3,num_ks), stat=ier)
  REQUIRE(ier==0)
  end if
  
#ifdef MPI
  if (.not. allocated(my_cg)) then
  allocate(my_cg(num_ks), stat=ier)
  REQUIRE(ier==0)
  end if
#endif

  if (.not. allocated(fractional_multipole)) then
  allocate(fractional_multipole(10,numatoms), stat=ier)
  REQUIRE(ier==0)
  end if

  if (.not. allocated(perm_F_field)) then 
...
# include "ew_parallel.h"
  ...
  call timer_stop_start(TIME_BSPL,TIME_FILLG)
  call pGM_RECIP_dipole_fill_grid(numatoms,Bspline_order,dr_order, &
                 is_polarizable, &
                 r_stack(p_fdip),  &
                 nfft1,nfft2,nfft3, &
                 nfftdim1,nfftdim2,nfftdim3,Q1,&
...
!-------------------------------------------------------------------------------
! So far it is hardwired for quadrupoles. Will need to remove extras
!-------------------------------------------------------------------------------
  ...
  do im = 1, nmine
    ...
    do ith3 = 1, Bspline_order
      ...
      if ( k >= kbot .and. k <= ktop ) then
        ...
        v2 = theta3(2,ith3,im) !2nd deriv of theta3
        j0 = j00
        do ith2 = 1,Bspline_order
          ...
          u2 = theta2(2,ith2,im) !2nd deriv of theta2
          ...
          term0 = Fmpole(Ind_000,im)*u0*v0 + &
                  ...
                  Fmpole(Ind_020,im)*u2*v0 + &
                  Fmpole(Ind_002,im)*u0*v2 + &
                  Fmpole(Ind_011,im)*u1*v1
          term1 = Fmpole(Ind_100,im)*u0*v0 + &
                  Fmpole(Ind_110,im)*u1*v0 + &
                  Fmpole(Ind_101,im)*u0*v1
          term2 = Fmpole(Ind_200,im)*u0*v0
          ...
          do ith1 = 1, Bspline_order
            ...
            t2 = theta1(2,ith1,im) !2nd deriv of theta1
...
!---------------------------------------------------------
subroutine pGM_RECIP_dipole_fill_grid(numatoms,Bspline_order,dr_order, &
                                  is_polarizable,fdip,  &
                                  nfft1,nfft2,nfft3, &
                                  nfftdim1,nfftdim2,nfftdim3,Q1, &
#ifdef MPI
...
# include "ew_parallel.h"
  ...
  return
end subroutine pGM_RECIP_dipole_fill_grid
!---------------------------------------------------------
...
!---------------------------------------------------------
! ZH: to replace previous bspline_fill_gen
...
!          300,030,003,210,201,120,021,102,012,111  in indices
  ...
  do im = 1, nmine
    ...
    do ith3 = 1,Bspline_order
      ...
      do ith2 = 1,Bspline_order
        ...
        do ith1 = 1,Bspline_order
          ...
          t_p(3) = t_p(3) + tq_p*theta1(3,ith1,im)
        end do !ith1 = 1,Bspline_order
    ...
    do m = 1,20
      Fperm_field(m,im) = tuv_p(m)
    end do
    Fperm_dipfield(1,im) = tuv_p(Ind_100)
    Fperm_dipfield(2,im) = tuv_p(Ind_010)
    Fperm_dipfield(3,im) = tuv_p(Ind_001)
  end do !im = 1,nmine
      ...
      if ( k >= kbot .and. k <= ktop ) then
       ...
       do ith2 = 1,Bspline_order
         ...
         do ith1 = 1,Bspline_order
           ...
           t_1(3) = t_1(3) + tq_1*theta1(3,ith1,im)
         end do !ith1 = 1,Bspline_order
...
# include "ew_parallel.h"
  ...
  F_dip_field(1:10,1:nmine) = F_dip_field(1:10,1:nmine) + F_field(1:10,1:nmine)
...
# include "ew_parallel.h"
  ...
  ! current index in each thread should be imy_cg + n - 1, while we save in the same place 
  ! in the frac mpole
