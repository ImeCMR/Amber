...
# include "pol_gauss_mpole_index.h" 
  ...
  public  pGM_DIRECT_perm_field,pGM_DIRECT_dip_dip_field,pGM_DIRECT_ene_frc, &
          pGM_DIRECT_dip_dip_field_short
...
subroutine pGM_alloc_dip_dip_tensors(list_tot, list_short_tot)
  ...
  integer :: list_tot, list_short_tot
    ...
    !if ( allocated(delvec_save) ) deallocate(delvec_save)
    !if ( allocated(Bfact_save) ) deallocate(Bfact_save)
    ...
    !allocate(delvec_save(4,list_tot))
    !allocate(Bfact_save(4,list_tot))

    !if (.not.allocated(dipole_dipole_ptrs))allocate(dipole_dipole_ptrs(natom+1))
    !if (.not.allocated(dipole_dipole_short_ptrs))allocate(dipole_dipole_short_ptrs(natom+1))
    !if (.not.allocated(last_ee_ptrs))allocate(last_ee_ptrs(natom))
...
subroutine pGM_DIRECT_calc_dipdip_field(num_tensor, &
               dipole_dipole_list,dipole_dipole_tensor, &
               ind_dip,dip_field)
  !use nblist, only: myindexlo,myindexhi
  ...
  do n = 1, num_tensor ! myindexlo,myindexhi
