...
#include "parallel.h"
    ...
    calcstd = 0
    dprc_nml%keepcrd = .false.
    if ( dprc_nml%al ) then
       if ( dprc_nml%avg ) then
          calcstd = 1
       else if ( dprc_nml%iskip <= 0 ) then
          calcstd = 1
       end if
    end if
...
!    if ( irank == 0 ) then
!        write(6,'(A,2es14.5,I5)')"ML AL max atomic frc stddev, thresh, skiplev:",maxstd,dprc_nml%althresh,dprc_nml%iskip
    !    end if
    dprc_nml%keepcrd = .false.
    if ( dprc_nml%al ) then
       !if ( maxstd > dprc_nml%althresh .and. dprc_nml%iskip <= 0 ) then
       if ( dprc_nml%iskip <= 0 ) then
          dprc_nml%keepcrd = .true.
          dprc_nml%iskip = dprc_nml%alskip
          if ( irank == 0 ) then
          ...
          dprc_nml%keepcrd = .false.
          if ( dprc_nml%iskip <= 0 .and. .not. dprc_nml%avg ) then
             dprc_nml%iskip = dprc_nml%alskip
          end if
          !if ( irank == 0 ) then
          !write(6,'(2es13.4)')maxstd,dprc_nml%althresh
          !end if
       endif
#ifdef MPI
    !call mpi_bcast(dprc_nml%keepcrd, 1, MPI_LOGICAL, 0, commsander, ier)
    !call mpi_bcast(dprc_nml%iskip, 1, MPI_INTEGER, 0, commsander, ier)
#endif
    end if
    
    dprc_nml%iskip = dprc_nml%iskip - 1
