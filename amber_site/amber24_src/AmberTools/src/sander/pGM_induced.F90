...
module pol_gauss_induced
  ...
  public pGM_INDUCED_eval,pGM_INDUCED_readparm, &
...
# include "parallel.h"
  ...
  call mpi_bcast(is_polarizable,num_atoms,MPI_LOGICAL,0,commsander,ier)
...
function pGM_INDUCED_readparm(nf,num_atoms)
  ...
  integer :: n,ier,dim1
  _REAL_, parameter :: polmin = 1.0d-6
...
# include "parallel.h"
  ...
  call pGM_NonBond_perm_fields(numatoms, is_polarizable, crd, x, ipairs, zv)
...
# include "parallel.h"
  ...
  REQUIRE(ier==0)
  allocate(ip(3*numpolar), stat = ier)
  REQUIRE(ier==0)
...
# include "ew_parallel.h"
  ...
  ! its diagonal elements are 1/pol (1/polarizability)
  call pGM_NonBond_dip_dip_fields(numatoms, x, xv, zv)
  ! xv: induced dipole(the same now), zv: dipole field
