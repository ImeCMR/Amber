...
contains
  ...
  !! @param[in] stoponfail stop calculation if it does not converge.
  !! @param[in] tolerance Convergence tolerances. There should be one
  !!          tolerance per closure in the closure list.
  function rism3d_calculateSolution(this, ksave, kshow, maxSteps, stoponfail, tolerance) result(success)
    ...
    if (this%nsolution == 0 .or. this%ncuvsteps == 0) then
       do iclosure = 1, size(this%closureList)
          ...
          call solve3DRISM(this, ksave, kshow, maxSteps, stoponfail, tolerance(iclosure))
       ...
       call timer_start(TIME_RXRISM)
       call solve3DRISM(this, ksave, kshow, maxSteps, stoponfail, tolerance(size(tolerance)))
       call timer_stop(TIME_RXRISM)
    end if
  ...
  !! @param[in] ksave save itermediate results every ksave interations (0 means no saves)
  !! @param[in] kshow print parameter for relaxation steps every kshow iteration (0 means no saves)
  !! @param[in] maxSteps maximum number of rism relaxation steps
  !! @param[in] stoponfail stop calculation if it does not converge.
  !! @param[in] tolerance convergence tolerance
  function rism3d_calculateSolution_dT(this, kshow, maxSteps, stoponfail, tolerance) result(success)
    implicit none
#if defined(MPI)
    include 'mpif.h'
#endif /*defined(MPI)*/
    ...
    if (this%failure) then
       success = .false.
       call rism_report_warn('Failure solving 3D-RISM equation, skipping temperature derivative.')
       return
    end if
    success = .true.
    ...
    call timer_start(TIME_RXRISM)
    call solve3DRISM_dT(this, kshow, maxSteps, stoponfail, tolerance)
    call timer_stop(TIME_RXRISM)
    if (this%failure) then
       success = .false.
       return
  ...
  function rism3d_excessChemicalPotential(this, o_lr) result(excessChemicalPotential)
    ...
    if (this%failure) then
       excessChemicalPotential = HUGE(1d0)
       return
    end if
  ...
  function rism3d_solvationEnergy_tot(this, o_lr) result(excessChemicalPotential_dT)
    ...
    if (this%failure) then
       excessChemicalPotential_dT = HUGE(1d0)
       return
    end if
...
!!!!!
  ...
  function getMinBoxLength(this) result ( boxLength)
    ...
    call rism3d_solute_translate(this%solute, -translation)
  ...
  subroutine reallocateBox(this, ngr, grdspc)
    ...
    this%guv => safemem_realloc(this%guv, this%grid%totalLocalPointsK, int(this%solvent%numAtomTypes, kind=int64), &
         o_preserve = .false., o_aligned = .true.)
    this%huv => safemem_realloc(this%huv, this%grid%totalLocalPointsK, int(this%solvent%numAtomTypes, kind=int64), &
         o_preserve = .false., o_aligned = .true.)
    this%cuvresWRK => safemem_realloc(this%cuvresWRK, this%grid%totalLocalPointsR, int(this%solvent%numAtomTypes, kind=int64), &
         int(this%NVec, kind=int64), .false.)
  ...
  subroutine reallocateBox_dT(this)
    ...
    this%cuvresWRK_dT => safemem_realloc(this%cuvresWRK_dT, this%grid%totalLocalPointsR, &
         int(this%solvent%numAtomTypes, kind=int64), int(this%NVec, kind=int64), .false.)
    ...
    this%cuvk => safemem_realloc(this%cuvk, this%grid%totalLocalPointsK, int(this%solvent%numAtomTypes, kind=int64), &
         o_preserve = .false., o_aligned = .true.)
    this%guv_dT => safemem_realloc(this%guv_dT, this%grid%totalLocalPointsK, int(this%solvent%numAtomTypes, kind=int64), &
         o_preserve = .false., o_aligned = .true.)
    this%huv_dT => safemem_realloc(this%huv_dT, this%grid%totalLocalPointsK, int(this%solvent%numAtomTypes, kind=int64), &
         o_preserve = .false., o_aligned = .true.)
...
#  endif /*USE_MPI_IN_PLACE*/
  ...
  subroutine interpolateSolventSusceptibility(this, xvv, xvva)
    ...
    integer(kind=int64) :: iwn, igk, igk1
    integer :: iv1, iv2
    _REAL_ :: err
    ...
    do iwn = 1, this%grid%waveNumberArraySize
       ...
       igk1 = 1 ! silence warning, igk is always > 1, so this value is set
  ...
  subroutine centerSolute(this, origin_o, translation_o)
    ...
    if (this%centering /= 0) then
       ...
       call rism3d_solute_translate(this%solute, origin - this%solute%centerOfMass)
    end if
  ...
  subroutine solve3DRISM(this, ksave, kshow, maxSteps, stoponfail, tolerance)
    use mdiis_c
    use rism3d_restart
    use ieee_arithmetic
    implicit none
#include "def_time.h"
    ...
    integer(kind=int64) :: ig, igr
    integer :: iv, iv2, istep
    _REAL_ :: residual = 0
    character(len=256) :: temp_message
    ...
    do istep = 1, maxSteps
       ...
       ! Exiting relaxation loop on convergence or error.
       if (converged .or. ieee_is_nan(residual)) then
          exit
       end if
    end do
    ...
    if (.not. converged ) then
       ...
       if( .not. ieee_is_nan(residual)) then
          write(temp_message,'(a,i5)') &
               'RXRISM: reached limit # of relaxation steps: ', maxSteps
       else
          write(temp_message, '(a)') 'RXRISM: NaN residual detected, aborting.'
       end if
       if (stoponfail)then
          call rism_report_error(temp_message)
       else
          call rism_report_warn(temp_message)
       end if
    else
       first = .false.
...
#include "def_time.h"
    ...
    integer(kind=int64) :: ig1, iga, igk
    integer :: iv, iv1, iv2, igx, igy, igz
#ifdef FFW_THREADS
  ...
  !! @param[in] stoponfail stop calculation if it does not converge.
  !! @param[in] tolerance Tolerance in.
  subroutine solve3DRISM_dT(this, kshow, maxSteps, stoponfail, tolerance)
...
#include "def_time.h"
    ...
    integer :: iatv, igx, igy, igz
    integer(kind=int64) :: igk
    logical ::  found, converged = .false.
    integer(kind=int64) :: ig
    integer :: iv, iv2, istep
    _REAL_ ::  residual = 0
    character(len=256) temp_message
    ...
    if (.not. converged ) then
       ...
       if( .not. ieee_is_nan(residual)) then
          write(temp_message,'(a,i5)') &
               'RXRISMDT: reached limit # of relaxation steps: ', maxSteps
       else
          write(temp_message, '(a)') 'RXRISMDT: NaN residual detected, aborting.'
       end if
       if (stoponfail)then
          call rism_report_error(temp_message)
       else
          call rism_report_warn(temp_message//" Results should be ignored.")
       end if
    else
       first = .false.
       if (this%mpirank == 0 .and. this%verbose >= 1) then
...
!$omp end parallel do
  ...
  subroutine rism3d_map_site_to_site_flat(this,thermo_map_flat, center_site)
    ...
    integer(kind=int64) :: i, iga, ig1
    integer :: iv1, iv2, igz, igy, igx
  ...
  subroutine rism3d_map_site_to_site_3D(this,thermo_map, center_site)
    ...
    thermo_map_flat => safemem_realloc(thermo_map_flat, this%grid%totalLocalPointsK, int(this%solvent%numAtomTypes, kind=int64), &
         o_preserve = .false., o_aligned = .true.)
    do iv1 = 1, this%solvent%numAtomTypes
    ! do i = 1, size(other_sites)
       ! iv1 = other_sites(i)
