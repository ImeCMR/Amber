...
module rism3d_solute_c
  ...
  type rism3d_solute
     ...
     !> translation from original coordinates
...
contains
  ...
  subroutine rism3d_solute_setCoord_ratu(this, ratu)
    use rism_util, only : findCenterOfMass
    implicit none
    type(rism3d_solute), intent(inout) :: this
    ...
    this%position = ratu
    call findCenterOfMass(this%position, this%centerOfMass, &
            this%mass, this%numAtoms)
  ...
  subroutine rism3d_solute_setCoord_crd(this, crd)
    use rism_util, only : freeUnit, findCenterOfMass
    implicit none
    type(rism3d_solute), intent(inout) :: this
  ...
  !> translate the coordinates of the atoms a fixed amount.
  !! @param[inout] this Solute object
  !! @param[in]    vector  :: translation 3-vector
  subroutine rism3d_solute_translate(this, vector)
    use rism_util, only : translate
    implicit none
    type(rism3d_solute), intent(inout) :: this
    _REAL_, intent(in) :: vector(3)
    call translate(this%position, this%numAtoms, vector)
    this%translation = vector
  end subroutine rism3d_solute_translate
