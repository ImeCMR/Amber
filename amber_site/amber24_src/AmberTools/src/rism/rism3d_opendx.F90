...
contains
  ...
  subroutine rism3d_opendx_write (file, data, grid, solute, o_rank, o_nproc, o_comm)
    use rism_util, only : freeUnit, rmExPrec, findCenterOfMass
    use rism3d_grid_c
    use rism3d_solute_c
    implicit none
#if defined(MPI)
    ...
    _REAL_ :: nCellVec(1,3), temp(1,3), currentCOM(3)
    ...
    if (rank == 0) then
       ...
       if (grid%periodic) then
          ...
          write(unit, "(a, 3(f15.8))") "origin  ", temp(1,:) - solute%translation
          ...
          write(unit, "(a, 3(f15.8))") "origin ", -solute%translation
