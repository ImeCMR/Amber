...
module rism3d_grid_c
  ...
  type rism3d_grid
     ...
     integer(kind=int64) :: totalLocalPointsR
     !> Total number of MPI local k-space grid points.
     integer(kind=int64) :: totalLocalPointsK
     !> Total number of global r-space grid points.
     integer(kind=int64) :: totalGlobalPointsR

     !> Number of indices in array of wave vector magnitudes.
     integer(kind=int64) :: waveNumberArraySize
     ...
     !! number value.
     integer(kind=int64), pointer :: waveVectorWaveNumberMap(:) => NULL()
...
contains
  ...
  subroutine rism3d_grid_resize(this, gridSpacing, globalDimsR, &
    ...
    this%waveVectors => safemem_realloc(this%waveVectors, 3_int64, this%totalLocalPointsK / 2, .false.)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  subroutine setup_wavevector(this)
    ...
    integer :: lgx, lgy, lgz
    integer :: igx, igy, igz
    integer(kind=int64) :: igk, igs
    _REAL_ ::  waveX, waveY, waveZ
    _REAL_ :: waveVector2, largestWaveVector2
    integer(kind=int64), pointer :: waveVectorToWaveVector2Map(:) => NULL()
