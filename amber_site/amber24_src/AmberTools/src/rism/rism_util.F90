...
!! @param[inout] ratu   :: the x,y,z position of each solute atom.  This is modified.
!! @param[in]    numAtoms   :: the number of solute atoms
...
subroutine indexArray(val, ptr, n)
  ...
  integer(kind=int64), intent(out) :: ptr(n)
  integer(kind=int64) :: i, temp
  ...
  do i = n, 1, -1
     ...
     call siftDown(val(1:i-1), ptr(1:i-1), 1_int64, i-1, n)
  end do
...
subroutine siftDown(val, ptr, i, n, m)
  implicit none
  integer(kind=int64), intent(in) :: i,n,m
  _REAL_, intent(in) :: val(m)
  integer(kind=int64), intent(inout) :: ptr(m)
  integer(kind=int64) :: root,child
