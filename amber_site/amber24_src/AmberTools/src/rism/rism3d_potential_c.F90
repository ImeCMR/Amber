...
#include "def_time.h"
  type rism3d_potential
     ...
     !> cache for long-range integrals, which can be expensive for
     !> large solutes
     _REAL_, pointer :: cached_h2(:) => NULL()
     _REAL_, pointer :: cached_hc(:) => NULL()
     _REAL_, pointer :: cached_hn(:) => NULL()
  ...
  private uvLJrEwaldPotentialWithMinimumImage, uvParticleMeshRecipEwaldPotential
contains
  ...
  subroutine rism3d_potential_new(this, grid, solv, solu, cut, fft, periodicPotential, &
    ...
    this%cached_h2 => safemem_realloc(this%cached_h2, this%solvent%numAtomTypes)
    this%cached_h2 = HUGE(1d0)
    this%cached_hc => safemem_realloc(this%cached_hc, this%solvent%numAtomTypes)
    this%cached_hc = HUGE(1d0)
    this%cached_hn => safemem_realloc(this%cached_hn, this%solvent%numAtomTypes)
    this%cached_hn = HUGE(1d0)
  ...
  subroutine rism3d_potential_calc(this, ljTolerance)
    ...
    if (ubound(this%uuv, 1) /= this%grid%localDimsR(1) .or. &
       ...
       this%uuv1d => safemem_realloc(this%uuv1d, this%grid%totalLocalPointsK, &
            int(this%solvent%numAtomTypes, kind=int64), o_preserve = .false., o_aligned = .true.)
    end if
  ...
  subroutine  rism3d_potential_dcf_tcf_long_range_asymptotics(this)
    ...
    _REAL_ :: solutePosition(3), soluteQ
  ...
  subroutine dcf_tcf_asymptotics_treecode(this)
    ...
    integer(kind=8) :: numSources, numTargets
  ...
  subroutine uvCoulombicPotential_treecode(this, ucu)
    ...
    call TREECODE(this%solute%position, &           !Array of source charges
            ...
            this%treeCoulombMAC, &
            this%treeCoulombN0, 2, 0d0, 0d0, 0d0)   !2 is Coulombic potential
  ...
  subroutine rism3d_potential_int_h2_hc(this, h2, hc)
    ...
    if( this%cached_h2(1) .ne. HUGE(1d0) .and. &
         this%cached_hc(1) .ne. HUGE(1d0) ) then
       h2 = this%cached_h2
       hc = this%cached_hc
       return
    end if       
  ...
  function rism3d_potential_int_hn(this, power) result(integral)
    ...
    if( this%cached_hn(1) .ne. HUGE(1d0) ) then
       integral = this%cached_hn
       return
    end if       
    integral = 0d0
  ...
  if (safemem_dealloc(this%cached_h2) /= 0) &
         call rism_report_error("cached_h2 deallocation failed")
  if (safemem_dealloc(this%cached_hc) /= 0) &
         call rism_report_error("cached_hc deallocation failed")
  if (safemem_dealloc(this%cached_hn) /= 0) &
         call rism_report_error("cached_hn deallocation failed")
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  subroutine uvLennardJonesPotentialWithMinimumImage(this, ulj)
    ...
    integer :: igx, igy, igz, iu, iv, id, cnt
    integer(kind=8) :: ig
    ! Grid point position.
...
!$omp end parallel do
  ...
  subroutine uvParticleMeshRecipEwaldPotential(this, ucu)
    ...
    integer(kind=int64) :: igx, igy, igz
    integer(kind=int64) :: igk, igk_fort, igk_cpp, igk_fort_global, igk_cpp_global
    integer :: id
    integer :: iu, iv
