...
//-----------------------------------------------------------------------------
#define NUM_WARP (THREAD_COUNT / GRID)
...
#endif
{
  ...
  __shared__ int sysID, atomStart, atomEnd, natom, ntypes, stepidx;
  __shared__ int bondStart, bondPos, bondEnd, randStart, randEnd;
  __shared__ int anglStart, anglPos, anglEnd;
  __shared__ int diheStart, dihePos, diheEnd;
  __shared__ int cnstStart, cnstPos, cnstEnd;
  __shared__ int tileStart, tilePos, tileEnd, gbReffPos, gbDRadPos;
  __shared__ int attnStart, attnPos, attnEnd;
  __shared__ float sysSdfac;
#ifdef GO_RATTLE
  __shared__ float *xprvcrd, *yprvcrd, *zprvcrd;
#endif
  // These arrays will be too big for the compiler to stuff into registers.
  __shared__ int ixfrc[ATOM_LIMIT], iyfrc[ATOM_LIMIT];
  __shared__ int izfrc[ATOM_LIMIT];
#ifdef COMPUTE_ENERGY
  __shared__ int sBondNrg[NUM_WARP];
  __shared__ int sAnglNrg[NUM_WARP];
  __shared__ int sDiheNrg[NUM_WARP];
  __shared__ int sSolvNrg[NUM_WARP];
  __shared__ int sElecNrg[NUM_WARP];
  __shared__ int sVdwNrg[NUM_WARP];
  __shared__ float sKineNrg[NUM_WARP];
#endif
  __shared__ float xcrd[ATOM_LIMIT], xvel[ATOM_LIMIT];
  __shared__ float ycrd[ATOM_LIMIT], yvel[ATOM_LIMIT];
  __shared__ float zcrd[ATOM_LIMIT], zvel[ATOM_LIMIT];
  __shared__ int psi[ATOM_LIMIT], sumdeijda[ATOM_LIMIT];
  __shared__ float reff[ATOM_LIMIT];
  ...
  if (sysID < cGms.nsys) {
    ...
    if (threadIdx.x == 0) {
      ...
      atomStart = limits.x;
      atomEnd   = limits.y;
      limits = cGms.DVCbondReadLimits[sysID];
      bondStart = limits.x;
      bondEnd   = limits.y;
      ...
      diheStart = limits.x;
      diheEnd   = limits.y;
      int4 limits1 = cGms.DVCnbReadLimits[sysID];
      tileStart = limits1.x;
      tileEnd   = limits1.y;
      attnStart = limits1.z;
      attnEnd   = limits1.w;
      if (cGms.Tstat.active == 1) {
        limits = cGms.DVCprngReadLimits[sysID];
        randStart = limits.x;
        randEnd   = limits.y;
    ...
    int warp_id = threadIdx.x >> GRID_BITS;
    ...
    WarpREDUCE(icenz);
    if (tgx == 0) {
      sBondNrg[warp_id] = icenx;
      sAnglNrg[warp_id] = iceny;
      sDiheNrg[warp_id] = icenz;
    }
    __syncthreads();
    if (warp_id == 0) {
      icenx = (tgx < NUM_WARP) ? sBondNrg[tgx] : 0;
      iceny = (tgx < NUM_WARP) ? sAnglNrg[tgx] : 0;
      WarpREDUCE(icenx);
      WarpREDUCE(iceny);
      if (tgx == 0) {
      ...
      icenz = (tgx < NUM_WARP) ? sDiheNrg[tgx] : 0;
      WarpREDUCE(icenz);
      if (tgx == 0) {
    ...
    while (stepidx < cGms.ntpr) {
	  ...
	  if (threadIdx.x < natom) {
          ...
          int warpCnstPos;
          if (tgx == 0) {
            warpCnstPos = atomicAdd((int*)&cnstPos, GRID);
          }
          warpCnstPos = __shfl_sync(0xffffffff, warpCnstPos, 0, GRID);
...
#  endif
          ...
          __syncthreads();
          if (tgx == 0) {
            warpWorkPos = atomicAdd((int*)&gbReffPos, GRID);
          }
          warpWorkPos = __shfl_sync(0xffffffff, warpWorkPos, 0, GRID);
          while (warpWorkPos < tileEnd) {
...
#ifdef PREFER_AUTO_L1
            uint2 pairmask = cGms.DVCpairBitMasks[warpWorkPos + tgx];
#else
            uint2 pairmask = __ldg(&cGms.DVCpairBitMasks[warpWorkPos +
                                                                tgx]);
#endif
            int katm = (pairmask.x >> 24)*HALFGRID + (tgx & HALFGRID_MASK);
            int matm = ((pairmask.x >> 16) & 0xff)*HALFGRID + (tgx & HALFGRID_MASK);
            // Initialize atoms and forces

            int loadatm    = (tgx < HALFGRID) ? katm : matm;
            ...
            int ondiagonal = ((katm >> GRID_BITS_DIAG) == (matm >> GRID_BITS_DIAG));

            // Accumulate psi
            float psiacc = (float)0.0;
            int j;
            #pragma unroll 4
            for (j = 0; j < QUARTGRID; j++) {
              int crdSrcLane = (tgx <  HALFGRID)*(tgx + HALFGRID - j) +
                               (tgx >= HALFGRID)*(tgx - (HALFGRID + QUARTGRID) + j);
              crdSrcLane += HALFGRID * ((crdSrcLane < HALFGRID && tgx <  HALFGRID) +
                                  (crdSrcLane <  0 && tgx >= HALFGRID));
              float oxcrd = __shfl_sync(0xffffffff, txcrd, crdSrcLane, GRID);
              float oycrd = __shfl_sync(0xffffffff, tycrd, crdSrcLane, GRID);
              float ozcrd = __shfl_sync(0xffffffff, tzcrd, crdSrcLane, GRID);
              float obrad = __shfl_sync(0xffffffff, tbrad, crdSrcLane, GRID);
              float ofs   = __shfl_sync(0xffffffff, tfs,   crdSrcLane, GRID);
              int   oneck = __shfl_sync(0xffffffff, tneck, crdSrcLane, GRID);
              ...
              int frcRetLane = (tgx <  HALFGRID)*(tgx + (HALFGRID + QUARTGRID) - j) +
                               (tgx >= HALFGRID)*(tgx - HALFGRID + j);
              frcRetLane += HALFGRID * ((frcRetLane >= HALFGRID && tgx >= HALFGRID) -
                                  (frcRetLane >= GRID && tgx <  HALFGRID));
              psitmp += __shfl_sync(0xffffffff, psiret, frcRetLane, GRID);
              psiacc += psitmp;
            }
...
#  endif
          ...
          if (tgx == 0) {
            warpWorkPos = atomicAdd((int*)&tilePos, GRID);
          }
          warpWorkPos = __shfl_sync(0xffffffff, warpWorkPos, 0, GRID);
#ifdef COMPUTE_ENERGY
          int uelec = 0;
          ...
          while (warpWorkPos < tileEnd) {
            ...
            float diagfac = ((katm >> GRID_BITS_DIAG) == (matm >> GRID_BITS_DIAG)) ?
                            (float)0.5 : (float)1.0;
#endif
            ...
            for (j = 0; j < QUARTGRID; j++) {
              ...
              float oycrd = __shfl_sync(0xffffffff, tycrd, crdSrcLane, GRID);
              float ozcrd = __shfl_sync(0xffffffff, tzcrd, crdSrcLane, GRID);
              float oqval = __shfl_sync(0xffffffff, tqval, crdSrcLane, GRID);
              int   oljid = __shfl_sync(0xffffffff, tljid, crdSrcLane, GRID);
              ...
              float r2 = dx*dx + dy*dy + dz*dz;
#ifdef GBSOLVENT
              float oreff = __shfl_sync(0xffffffff, treff, crdSrcLane, GRID);
              int   oatom = __shfl_sync(0xffffffff, loadatm, crdSrcLane, GRID);
...
#  endif
              ...
              if ((pairmask.y >> (crdSrcLane - HALFGRID*(tgx < HALFGRID))) & 0x1) {
...
#  endif
              ...
              frcRetLane += HALFGRID * ((frcRetLane >= HALFGRID && tgx >= HALFGRID) -
                                  (frcRetLane >= GRID && tgx <  HALFGRID));
              txfrc -= __shfl_sync(0xffffffff, xPairFrc, frcRetLane, GRID);
              tyfrc -= __shfl_sync(0xffffffff, yPairFrc, frcRetLane, GRID);
              tzfrc -= __shfl_sync(0xffffffff, zPairFrc, frcRetLane, GRID);
#ifdef GBSOLVENT
              tdeijda += __shfl_sync(0xffffffff, odeijda, frcRetLane, GRID);
#endif
            }
...
#  endif
          if (tgx == 0) {
            sElecNrg[warp_id] = uelec;
            sVdwNrg[warp_id] = uvdw;
#  ifdef GBSOLVENT
            sSolvNrg[warp_id] = usolv;
#  endif
          }
#endif
#ifdef GBSOLVENT
          ...
          if (tgx == 0) {
            warpWorkPos = atomicAdd((int*)&gbDRadPos, GRID);
          }
          warpWorkPos = __shfl_sync(0xffffffff, warpWorkPos, 0, GRID);
          while (warpWorkPos < tileEnd) {
            ...
            for (j = 0; j < QUARTGRID; j++) {
              ...
              int   oneck   = __shfl_sync(0xffffffff, tneck,  crdSrcLane, GRID);
              float obrad   = __shfl_sync(0xffffffff, tbrad,  crdSrcLane, GRID);
              float osumda  = __shfl_sync(0xffffffff, tsumda, crdSrcLane, GRID);
              float oinvrad = (float)1.0 / obrad;
              float dx = oxcrd - txcrd;
              float dy = oycrd - tycrd;
              float dz = ozcrd - tzcrd;
          ...
          if (tgx == 0) {
            warpWorkPos = atomicAdd((int*)&dihePos, GRID);
          }
          warpWorkPos = __shfl_sync(0xffffffff, warpWorkPos, 0, GRID);
#ifdef COMPUTE_ENERGY
          int udihe = 0;
...
#endif
        if (tgx == 0) {
          warpWorkPos = atomicAdd((int*)&attnPos, GRID);
        }
        warpWorkPos = __shfl_sync(0xffffffff, warpWorkPos, 0, GRID);
        ...
        if (i == 0) {
          ...
          WarpREDUCE(uvdwAttn);
          if (tgx == 0) {
            sElecNrg[warp_id] += uelecAttn;
            sVdwNrg[warp_id] += uvdwAttn;
          }
        }
#endif
...
#endif
        if (tgx == 0) {
          warpWorkPos = atomicAdd((int*)&bondPos, GRID);
        }
        warpWorkPos = __shfl_sync(0xffffffff, warpWorkPos, 0, GRID);
...
#endif
        if (tgx == 0) {
          warpWorkPos = atomicAdd((int*)&anglPos, GRID);
        }
        warpWorkPos = __shfl_sync(0xffffffff, warpWorkPos, 0, GRID);
...
#else
    if (warp_id == 0) {
      int utmp1 = (tgx < NUM_WARP) ? sBondNrg[tgx] : 0;
      int utmp2 = (tgx < NUM_WARP) ? sAnglNrg[tgx] : 0;
      int utmp3 = (tgx < NUM_WARP) ? sDiheNrg[tgx] : 0;
      float ukine = (tgx < NUM_WARP) ? sKineNrg[tgx] : (float)0.0;
      WarpREDUCE(ukine);
      WarpREDUCE(utmp1);
      WarpREDUCE(utmp2);
      WarpREDUCE(utmp3);
      if (tgx == 0) {
        cGms.Ubond.DevcData[(sysID * cGms.nsgmdout) + sgc] =
          (double)utmp1 / FPSCALEnrgHP;
        cGms.Uangl.DevcData[(sysID * cGms.nsgmdout) + sgc] =
          (double)utmp2 / FPSCALEnrgHP;
        cGms.Udihe.DevcData[(sysID * cGms.nsgmdout) + sgc] =
          (double)utmp3 / FPSCALEnrgHP;
        cGms.Ukine.DevcData[(sysID * cGms.nsgmdout) + sgc] =
        (double)ukine;
      }
    }
    else if (warp_id == 1) {
        int utmp1 = (tgx < NUM_WARP) ? sElecNrg[tgx] : 0;
        int utmp2 = (tgx < NUM_WARP) ? sVdwNrg[tgx] : 0;
        int utmp3 = (tgx < NUM_WARP) ? sSolvNrg[tgx] : 0;
        utmp1 >>= MP2LP_BITS;
        utmp2 >>= MP2LP_BITS;
        utmp3 >>= MP2LP_BITS;
        WarpREDUCE(utmp1);
