    ...
    mD( n, 0. )
{
  ...
  if ( n == 1 )
  ...
  else if ( n == 2 )
    {
      ...
      if ( dx <= 0 )
	{
	  std::cerr << "AkimaSpline::AkimaSpline xs must be ascending (2)"
		    << std::endl;
	  std::exit(EXIT_FAILURE);
	}
      else
	  ...
	  if ( dx <= 0 )
	    {
	      std::cerr << "AkimaSpline::AkimaSpline xs must be ascending (>2)"
			<< " i= " << i
			<< " dx = " << std::scientific << std::setw(12)
			<< std::setprecision(3) << dx
			<< " x[i] " << std::scientific << std::setw(12)
			<< std::setprecision(3) << xs[i]
			<< std::endl;
	      std::exit(EXIT_FAILURE);
	    }
...
double ccdl::AkimaSpline::GetDeriv( double const x ) const
{
  double v = 0;
  if ( x < mX[0] )
    {
  ...
  else if ( x > mX.back() )
    {
      ...
      int n = mX.size();
      int ifound = n-2;
      double dx = mX[n-1] - mX[n-2];
      v = mB[ifound] + 2*dx*mC[ifound] + 3*dx*dx*mD[ifound];
    }
  else
    {
      int ifound = FindIdx(x);
      double dx = x - mX[ifound];
      //double u = mY[ifound] + dx*(mB[ifound] + dx*(mC[ifound] + dx*mD[ifound]));
