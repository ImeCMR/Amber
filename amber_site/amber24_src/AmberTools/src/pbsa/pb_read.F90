...
!     end select
   ...
   if( idecomp > 0 ) then
      write(6, '(a,i3)') ' ERROR: decomposition not supported.', idecomp
      call mexit(6, 1)
   end if
...
subroutine pb_read
   ...
   use poisson_boltzmann, only : epsin, epsout, epsmem, smoothopt,   &
         ...
         level, lastp, savxm, savym, savzm, savxmym,  &
         ...
         fmiccg2, fold16, rxm, rym, rzm, rgox, rgoy, rgoz, &
         epslow, epshigh, epsmemb, prt_srfcrg

   use dispersion_cavity
   use solvent_accessibility
   use density_surface, only : mlses_rdfeature, mlses_rdfunction, &
         mlses_bench, bench_opt, mlses_opt, grad_opt
#ifdef SANDER
   use file_io_dat
#endif
   implicit none
...
#  include "files.h"
   ...
   namelist /pb/ epsin, epsout, epsmem, smoothopt, istrng, pbtemp,   &
      ...
      mlses_rdfeature, mlses_rdfunction, mlses_bench, bench_opt, &
      grad_opt, prt_srfcrg
#endif
   ...
   if ( membraneopt == 1 .or. membraneopt == 2 .or. membraneopt == 3) then
   ...
   else if ( membraneopt == 0 ) then
      epsmem = 1.0d0
      epsmemb = 1.0d0
   else
      write(6,'(a)') ' PB Bomb in pb_read(): membrane option not supported '
      call mexit(6, 1)
   end if
...
!     call mexit(6,1)
   ...
   if ( membraneopt == 0 ) then
      epslow = min(epsin,epsout)*0.999d0
      epshigh = max(epsin,epsout)*1.001d0
   else if ( membraneopt == 1 ) then
      epslow = min(epsin,epsmem,epsout)*0.999d0
      epshigh = max(epsin,epsmem,epsout)*1.001d0
   else if ( membraneopt == 2 .or. membraneopt == 3) then
      epslow = min(epsin,1.0d0*eps0,epsout)*0.999d0
      epshigh = max(epsin,80.0d0*eps0,epsout)*1.001d0
   end if
   ...
   ! it also turns off nonpolar solvent model calculations

   if ( phiout == 1 ) then
      outphi = .true.
      if ( radiopt /= 2 ) write(6,'(a,a,i6,f6.2)') &
      ' PB Info in pb_read(): radiopt is no longer enforced. Please make sure',&
      ' atomic radii and water probe are consistent with those in the visualization program.',&
      radiopt, dprob
      inp = 0
      donpsa = .false.
      npopt = 0
