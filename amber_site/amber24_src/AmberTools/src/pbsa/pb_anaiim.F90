...
subroutine pb_anaiim( npbstep,npbgrid,nstlim,atmfirst,atmlast,npbopt,solvopt,level,nfocus,bcopt,&
   ...
   if ( bcopt /= 2 .and. bcopt /= 6 ) then
      write(6,*) 'PB Bomb in pb_anaiim():bcopt can only be 2 or 6'
      call mexit(6,1)
   end if
...
subroutine pb_bndcnd( bv, chgrd )
   ...
   ! electrostatic focusing
      ...
      write(6, *) 'PB bomb in pb_anaiim(): unknown BC option', bcopt
      call mexit(6, 1)
   end if 
...
subroutine iim(gox,goy,goz,l,m,n,lvlset,insas,nbnd,iepsav,epsin,epsout,bv,u,u0,h,&
   ...
   ! The real nonzero number of coefficients is the one commented out. 
   ! However, for the sake of generating the incomplete LU matrix, we
   ! assume a complete seven-banded matrix for the 7-point
   ! stencil FD linear system.
   !nz_num = nz_num + 7*((l-2)*(m-2)*(n-2)-nbnd) + 6*2*((l-2)*(m-2)+(m-2)*(n-2)+(n-2)*(l-2)) + & 
   !         5*4*((l-2)+(m-2)+(n-2)) + 4*8
   nz_num = nz_num + 7*(l*m*n-nbnd)-2*(1+l+l*m)
   write(6,*) ' PB IIM: nonzero elements requested ', nz_num
...
!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
!+ first try the second-order approach 
subroutine irre31(l,m,n,h,hx,hy,hz,ifail,i0,j0,k0,info,bmax,b_in,b_out,x,y,z,phi,index, &
   ...
   parameter(im=37, ime=10, immax=im, ns=27, in=ns, &
...
!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
!+ next try the first-order approach 
subroutine irre32(l,m,n,h,hx,hy,hz,ifail, &
   ...
   integer,parameter :: im=31, ime=4, immax=im, ns=27, in=ns, &
         inmax=in, imnn=im+2*in, &
         lwar=3*inmax*inmax/2+10*inmax+2*immax+1, &
         liwar=in
   ...
   ! curvature would be zero for first-order approach
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
   ...
   do l = 1, nbnd
      ...
      !write(58,'(a,3(f20.15),2x)') "H"//str, cirreg(1,l)+0.49529492855072021, cirreg(2,l)-0.32362693548202515, cirreg(3,l)-0.2396472692489624

      ! convert the curvature to the angstrom unit
      ! most curvatures are negative according to the definition of level set function

      ! Do the surface regulation
      ! first the inward-facing curvature cannot be larger than that of the solvent probe,
      ! i.e. the solvent reentry.
      if ( bnd_deriv(1) > ONE/dprobh ) bnd_deriv(1) = ONE/dprobh
      if ( bnd_deriv(2) > ONE/dprobh ) bnd_deriv(2) = ONE/dprobh

      ! second if the grid spacing is already larger than the smallest atom,
      ! i.e. the hydrogen (0.70 A), then we will treat all surface
      ! (inward/outward-facing) as spheres.
      if ( (bnd_deriv(1)*bnd_deriv(2) > 0) .and. h > 0.7d0 ) then
         if ( abs(bnd_deriv(1)) <= abs(bnd_deriv(2)) ) then 
            cirreg(4:5, l)=bnd_deriv(1)/h
         else 
            cirreg(4:5, l)=bnd_deriv(2)/h
         end if
         cirreg(6, l) = ZERO
      else
...
subroutine find_projection(x,y,z,l,proj_point)
   ...
   middlelvlset = &
   find_lvlset(x-sign(ONE,nextlvlset)*gradient(1),y-sign(ONE,nextlvlset)*gradient(2),z-sign(ONE,nextlvlset)*gradient(3),l)
   indicator=0
   if ( nextlvlset*middlelvlset<=0 ) then
      indicator=1
      if ( nextlvlset>1 ) then
         x_proj=x-gradient(1)
         y_proj=y-gradient(2)
         z_proj=z-gradient(3)
      ...
      ! if the newton method fails, let's do bisection search.
      if ( (x_proj-x)**2+(y_proj-y)**2+(z_proj-z)**2 >= ONE ) indicator=0
   end if
   if (indicator==0) then ! bisection method
