...
subroutine pb_pcg(phi, xs)
   ...
   ! For converting _REAL_ to float as in wrapper.
   !real, dimension(l_xmymzm) :: x, b
   integer nz_num, ier
   integer :: ioffset(13)
   integer, allocatable :: ia(:), ja(:)
   real, allocatable :: sa(:), x(:), b(:)
   real :: saccept, snorm
   integer nb
...
#ifdef DIA
   nz_num = 11*l_xmymzm-2*(l_xm+l_xmym)
   if ( allocated(sa) ) then
       deallocate(sa, stat = ier); REQUIRE(ier==0)
   end if
   ...
   b(1:l_xmymzm) = l_bv(1:l_xmymzm)
   l_inorm = sum(abs(b(1:l_xmymzm)))

#ifdef CSR
   call setpcsr(l_xm,l_ym,l_zm,nz_num,sa,ia,ja)
   ja = ja - 1
   ia = ia - 1
   ! ---- CUSPARSE version
   !call cusparse_cg_wrapper(x, b, ja, ia, sa, l_xmymzm, nz_num, l_maxitn, saccept, l_itn, snorm)
   ! ---- CUSP version
   call cusp_cg_wrapper(x, b, ja, ia, sa, l_xmymzm, nz_num, l_maxitn, saccept, l_itn, snorm)
#endif
#ifdef DIA
   call setpdia(l_xm,l_ym,l_zm,ioffset,sa)
   call cusp_cg_wrapper(x, b, ioffset, sa, l_xm, l_ym, l_zm, nz_num, nb, l_maxitn, saccept, l_itn, snorm)
#endif
   l_norm = snorm
   ...
   if ( l_itn >= l_maxitn ) then
      ...
      xs(1:l_xmymzm) = x(1:l_xmymzm)
   end if

   ! deallocate arrays - R.Q.
#ifdef CSR
   if ( allocated(sa) ) then
       deallocate(sa, stat = ier); REQUIRE(ier==0)
   end if
...
#ifdef CSR
subroutine setpcsr(l,m,n,nz_num,a,ia,ja)
!
! ja is row index and ia is column index
!
   implicit none
   integer l,m,n,nz_num
   real :: a(nz_num)
...
end subroutine setpcsr
#endif

#ifdef DIA
subroutine setpdia(xm,ym,zm,offset,a)
   ...
   offset = (/xmym-xmymzm, -xmym, xm-xmym, -xm, 1-xm, -1, 0, 1, xm-1, xm, xmym-xm, xmym, xmymzm-xmym/)

   a(              1:xmymzm-2*xmym) = 0 ! where l_am6 was not initiallized
   a(xmymzm-2*xmym+1:   xmymzm    ) = -l_am6(   1-xmym:xmym          )
   a(       xmymzm+1: 2*xmymzm    ) = -l_am3(   1-xmym:xmymzm-xmym   )
   a(     2*xmymzm+1: 3*xmymzm    ) = -l_am5(1-xmym+xm:xmymzm-xmym+xm)
   a(     3*xmymzm+1: 4*xmymzm    ) = -l_am2(     1-xm:xmymzm-xm     )
   a(     4*xmymzm+1: 5*xmymzm    ) = -l_am4(   2-xmym:xmymzm-xmym+1 )
   a(     5*xmymzm+1: 6*xmymzm    ) = -l_am1(        0:xmymzm-1      )
   a(     6*xmymzm+1: 7*xmymzm    ) =  l_ad (        1:xmymzm        )
   a(     7*xmymzm+1: 8*xmymzm    ) = -l_am1(        1:xmymzm        )
   a(     8*xmymzm+1: 9*xmymzm    ) = -l_am4(        1:xmymzm        )
   a(     9*xmymzm+1:10*xmymzm    ) = -l_am2(        1:xmymzm        )
   a(    10*xmymzm+1:11*xmymzm    ) = -l_am5(        1:xmymzm        )
   a(    11*xmymzm+1:12*xmymzm    ) = -l_am3(        1:xmymzm        )
   a(    12*xmymzm+1:13*xmymzm    ) = -l_am6(        1:xmymzm        )

end subroutine setpdia
#endif
...
subroutine pb_cg(phi, xs)
   ...
   nz_num = 7*l_xmymzm-2*(1+l_xm+l_xmym) ! Same for CSR/DIA
   ...
   l_inorm = sum(abs(b(1:l_xmymzm)))
#ifdef CSR
   call setcsr(l_xm,l_ym,l_zm,nz_num,sa,ia,ja)
...
#endif
#ifdef DIA
   call setdia(l_xm,l_ym,l_zm,ioffset,sa)
   ! ---- CUSP version
   call cusp_cg_wrapper(x, b, ioffset, sa, l_xm, l_ym, l_zm, nz_num, nb, l_maxitn, saccept, l_itn, snorm)
#endif
   l_norm = snorm
...
#ifdef CSR
subroutine setcsr(l,m,n,nz_num,a,ia,ja)
!
! ja is row index and ia is column index
!
   implicit none
...
#ifdef DIA
subroutine setdia(xm,ym,zm,offset,a)
   ...
   offset = (/-xmym, -xm, -1, 0, 1, xm, xmym/)

   a(         1:  xmymzm) = -l_am3(1-xmym:xmymzm-xmym)
   a(  xmymzm+1:2*xmymzm) = -l_am2(  1-xm:xmymzm-xm  )
   a(2*xmymzm+1:3*xmymzm) = -l_am1(     0:xmymzm-1   )
   a(3*xmymzm+1:4*xmymzm) =  l_ad (     1:xmymzm     )
   a(4*xmymzm+1:5*xmymzm) = -l_am1(     1:xmymzm     )
   a(5*xmymzm+1:6*xmymzm) = -l_am2(     1:xmymzm     )
   a(6*xmymzm+1:7*xmymzm) = -l_am3(     1:xmymzm     )
