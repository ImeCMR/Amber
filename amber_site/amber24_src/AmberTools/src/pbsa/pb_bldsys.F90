...
subroutine pb_exmol( verbose,ifcap,mpdec,ipb,inp,savbcopt,saopt,sasopt,natom,&
   ...
   use poisson_boltzmann, only: ligand, eneopt, &
      mthick, outlvlset, outmlvlset, poretype, poreradi, &
   ...
   use density_surface, only: density_lvlset, mlses_bench, mlses_feature, mlses_density, irreg_init, num_irreg_grd, bndatm
...
#  include "pb_md.h"
   ...
   if ( memopt > 0 ) then
      ...
      inmem(0:xm+1,0:ym+1,0:zm+1) = 0
   end if

   if (ipb /= 4 .and. ipb /= 5 .and. ipb /= 6 .and. ipb /= 7 .and. ipb /= 8) then
   ...
   if ( level == 1 .and. .not. ligand .and. sasopt /= 3 ) then
      call pbtimer_start(PBTIME_PBSAS)
   ...
   if      ( sasopt == 1 ) then
      ...
      ! LX: modulized for more general surfaces as in multi-regions/sovlents probes
      call pbtimer_start(PBTIME_PBEXMOL_PARTA)
      insas(1:xmymzm_ext) = -4; atmsas(1:xmymzm_ext) = 0
      call pbtimer_stop(PBTIME_PBEXMOL_PARTA)
      ...
      ! XP: turned on density function to mark grid nodes in one step when sasopt == 2
      ! WMBS: added membrane support
      ! RQ: added ipb option for CUDA
      call pbtimer_start(PBTIME_PBEXMOL_PARTA)
      insas(1:xmymzm_ext) = -2
      call pbtimer_stop(PBTIME_PBEXMOL_PARTA)
      call density_lvlset(eneopt,memopt,outlvlset,outmlvlset,natom,xm,ym,zm,gox,goy,goz,h,dprob*rh,&
              cutfd,mzmin,mzmax,gcrd,radip3,poretype,poreradi,insas,inmem,lvlset,mlvlset,ipb)

   else if ( sasopt == 3 ) then

      ! Haixin: turned on machine-learned function to mark grid nodes in one step when sasopt == 3
      call pbtimer_start(PBTIME_PBEXMOL_PARTA)
      insas(1:xmymzm_ext) = -2; lvlset(1:xmymzm_ext) = 9999d0
      call pbtimer_stop(PBTIME_PBEXMOL_PARTA)
#if !defined SANDER && !defined LIBPBSA
      call pbtimer_start(PBTIME_PBEXMOL_PARTB)
      call mlses_feature(natom,xm,ym,zm,gox,goy,goz,h,dprob*rh,gcrd,radi,insas,lvlset)
      call pbtimer_stop(PBTIME_PBEXMOL_PARTB)
      call pbtimer_start(PBTIME_PBEXMOL_PARTC)
      call mlses_density(xm,ym,zm,insas,lvlset)
      call pbtimer_stop(PBTIME_PBEXMOL_PARTC)
#else
      write(6,'(a)') "PB Bomb in pb_exmol: sasopt=3 does not work in SANDER OR LIBPBSA"
      call mexit(6,1)
#endif
   ...
   if ( ( ipb == 2 .or. ipb == 4 .or. ipb == 5 ) .and. (sasopt == 0 .or. sasopt == 1) ) then
      ...
      if ( mlses_bench == 1 ) then
         ...
         call pbtimer_stop(PBTIME_PBEXMOL_PARTA)
#else
         write(6,'(a)') "PB Bomb in pb_exmol: sasopt=0/mlses_benchmark does not work in SANDER OR LIBPBSA"
         call mexit(6,1)
#endif
      end if
   end if
   ...
   ! RL: For ipb==2 applications, only the p3m option needs this.

   if ( ((ipb == 2 .and. eneopt ==4) .or. ipb == 4 .or. ipb == 5 .or. ipb == 6 .or. ipb == 7 .or. ipb== 8) .and. &
      ...
      if ( sasopt == 2 ) then
         ...
         if ( ipb == 6 ) then ! will need it with ipb=7 and 8 if forces are needed.
#if !defined SANDER && !defined LIBPBSA
...
#ifdef nvCUDA
            if (ipb /= 2) then
               call num_irreg_grd(xm,ym,zm,nbnd,h,iepsav,lvlset) ! R.Q.
            end if
#else
            call num_irreg_grd(xm,ym,zm,nbnd,h,iepsav,lvlset) ! R.Q.
#endif
         else if ( ipb /= 7 .and. ipb /= 8 ) then
            write(6,'(a)') "PB Bomb in pb_exmol: Uknown IPB for density surface runs"
            call mexit(6,1)
         end if
      else
         if ( ipb == 4 .or. ipb == 5) then
            call num_irreg_grd(xm,ym,zm,nbnd,h,iepsav,lvlset)
         else if ( ipb == 6 .or. ipb == 7 .or. ipb == 8 ) then
            write(6,'(a)') 'PB Bomb in pb_exmol: ipb=6/7/8 only work with the analytical density surface'
            call mexit(6,1)
         else
         ...
         if ( range3 >= 0.0d0 ) then
            ...
            do i = lowi, highi
               if ( insph(i,j,k) == dielsph ) then
                  ...
                  if ( dist < 1.d-9 ) then ! R. Luo. preventing overflow below
                     inatm(i,j,k) = iatm
                     cycle
                  end if
                  tmp = tmp / dist
...
subroutine assignlvlset(natom,xm,ym,zm,nbnd,dprob,h,gox,goy,goz,radi,&
   ...
   use solvent_accessibility, only: arccrd
   use density_surface, only: mlses_bench
   implicit none
   ...
   ! procedure repeats on most visited grid points, so an "if-cycle" is inserted
   ! to skip those done grid points.
   ! buffer is set to be dependent on mlses_bench to compute lvlset for as many
   ! grid points within SAS as possible.
   rh = 1.0d0 / h
   dist = dprob*rh
   if ( mlses_bench == 1 ) then
      buffer = 16
   ...
   do l = 1, nbnd
      do k = max(1,iepsav(3,l) - buffer), min(iepsav(3,l) + buffer, zm)
         do j = max(1,iepsav(2,l) - buffer), min(iepsav(2,l) + buffer, ym)
            do i = max(1,iepsav(1,l) - buffer), min(iepsav(1,l) + buffer, xm)
               ! if it is done already, skip
               if ( abs(u(i,j,k)) /= 9999.0d0 ) cycle
               ! skip those without owners for mlses training
               ! as we are using a very large buffer
               if ( mlses_bench == 1 .and. atmsas(i,j,k) == 0 ) cycle
...
subroutine epsmap( ipb,sasopt,smoothopt,memopt,newown,natom,&
   ...
   use poisson_boltzmann, only: mthick, mzctr, epslow, epshigh, eps0 ! R.Q. for z-depth dependent eps
...
#  include "pb_constants.h"
   ...
   _REAL_ epsint, epsint0, epsmemz, pcfit, zdpt, adjt
   _REAL_ frac, rh
   !_REAL_ epslow, epshigh

   ! ipb2 surface rendering statements
   !character(10) str
   !open(unit=58,file='interface.dot')
   !write (58, *) nbndx+nbndy+nbndz
   !write (58, '("DOTS")')

   rh = ONE/h

   if ( (sasopt == 2 .or. sasopt == 3) .and. memopt > 0 ) then
      write(6,'(a)') ' PB Bomb in pb_epsmap(): The density/mlses function option for'
   ...
   do k = 0, zm;
      ...
      epsmemz = epsmem
      ! z-dependent mem eps
      if (memopt == 2 .or. memopt == 3) then
          pcfit = 20.0d0 - 2.5d0 ! fitted range, not adjustable
          zdpt = abs(k - mzctr) * h ! z-depth relative to m-center
          adjt = mthick * 0.5d0 - pcfit ! adjustable mem core, half
          ! This handles any mthick either smaller or larger than the fitted range
          if (memopt == 2) then
             epsmemz = 1.0d0 / epspchip(zdpt - adjt + 2.5d0) ! offsetting coordinate to 2.5
          else if (memopt == 3) then
             epsmemz = 1.0d0 / epsspline(zdpt - adjt + 2.5d0) ! offsetting coordinate to 2.5
          end if
          !write(55,'(a,2i4,6f12.4)') 'k, zm, pcfit, mzctr, zdpt, adjt, oz, epsmemz', k, zm, pcfit, mzctr, zdpt, adjt, &
          !    zdpt - adjt + 2.5d0, epsmemz
          epsmemz = epsmemz * eps0 ! restore absolute dielectric unit
      end if
   ...
   do j = 0, ym; do i = 0, xm
      ...
      if ( aa > 0 .or. bb > 0 ) then
         ...
         if ( j == 0 .or. k == 0 ) then ! note this edge is not defined
            ...
            call epsfracx(i,j,k,a,b,a1,b1,rh,epsint,epsin,epsmemz) ! z-dependent epsmem
            epsx(i,j,k) = epsint
            ...
            epsx(i,j,k) = epsmemz ! z-dependent epsmem
         ...
         if ( j == 0 .or. k == 0 ) then ! note this edge is not declared
            ...
            if ( level == nfocus ) then
               ...
               if ( (ipb == 1 .or. ipb == 2 .or.&
                  (ipb == 3 .and. (a == 2 .or. b == 2)) ) .and. sasopt /= 2 .and. sasopt /= 3 ) &
                  call epsfracx  (i,j,k,a,b,a1,b1,rh,epsint,epsin,epsout  )
               if ( ((ipb == 2 .or. (ipb == 3 .and. ( a == 1 .or. b == 1)) ) .and. &
                  nbndx > x_flag) .or. (ipb == 2  .and. (sasopt == 2 .or. sasopt == 3)) ) then
...
10    continue
      ...
      if ( aa > 0 .or. cc > 0 ) then
         ...
         if ( i == 0 .or. k == 0 ) then ! note this edge is not declared
            ...
            call epsfracy(i,j,k,a,c,a1,c1,rh,epsint,epsin,epsmemz) ! z-dependent epsmem
            epsy(i,j,k) = epsint
            ...
            epsy(i,j,k) = epsmemz ! z-dependent epsmem
         end if ! the rest is to use the default solvent value
      ...
      ! c. edges flanked by two points of different signs are fraction edges
         ...
         if ( i == 0 .or. k == 0 ) then
            ...
            if ( level == nfocus ) then
               ! for all smoothopt's do the edge fraction calculation
               if ( (ipb == 1 .or. ipb == 2 .or.&
                    (ipb == 3 .and. (a == 2 .or. c == 2)) ) .and. sasopt /= 2 .and. sasopt /= 3) &
                  call epsfracy  (i,j,k,a,c,a1,c1,rh,epsint,epsin,epsout  )
               if ( ((ipb == 2 .or. (ipb == 3 .and. ( a == 1 .or. c == 1)) ) .and.&
                    nbndy > y_flag) .or. (ipb == 2 .and. (sasopt == 2 .or.  sasopt == 3)) ) then
...
20    continue
      ...
      if ( aa >0 .or. dd >0 ) then
         if ( i == 0 .or. j == 0 ) then ! note this edge is not declared
            ...
            call epsfracz(i,j,k,a,d,a1,d1,rh,epsint,epsin,epsmemz) ! z-dependent epsmem
            epsz(i,j,k) = epsint
            ...
            epsz(i,j,k) = epsmemz ! z-dependent epsmem
            ...
            if ( aa == 1 .and. k == floor(mzmax) ) then
               frac = REAL(k+1) - mzmax
               epsint = (epsout*epsmemz)/(epsmemz*(1.0d0-frac) + epsout*frac)
               epsz(i,j,k) = epsint
            else if ( dd == 1 .and. k+1 == ceiling(mzmin)) then
               frac = mzmin - REAL(k)
               epsint = (epsout*epsmemz)/(epsmemz*(1.0d0-frac) + epsout*frac) ! z-dependent epsmem
               epsz(i,j,k) = epsint
            end if ! the rest is to use the default solvent value
         end if
      ...
      ! c. edges flanked by two points of different signs are fraction edges
         ...
         if ( i == 0 .or. j == 0 ) then
            ...
            if ( level == nfocus ) then
               ! for all smoothopt's do the edge fraction calculation
               if ( (ipb == 1 .or. ipb == 2 .or.&
                    (ipb == 3 .and. (a == 2 .or. d == 2)) ) .and. sasopt /= 2 .and. sasopt /= 3 ) &
                  call epsfracz  (i,j,k,a,d,a1,d1,rh,epsint,epsin,epsout  )
               if ( ((ipb == 2 .or. (ipb == 3 .and. ( a == 1 .or. d == 1)) ) .and. nbndz > z_flag) .or.&
                    (ipb == 2 .and. (sasopt == 2 .or. sasopt == 3)) ) then
...
30    continue
   ...
   do k = 0, zm; do j = 0, ym; do i = 0, xm
      !Test z-dependent eps
      !if ( i == 1 .and. j == 1 ) then
      !  write(66,'(a,3i6,9e12.3)') 'i, j, k, epsin, epsmem, epsmemz, epsout, kh, mzctr, epslow, epshigh, epsz:', &
      !      i, j, k, epsin, epsmem, epsmemz, epsout, k*h, mzctr, epslow, epshigh, epsz(i,j,k)
      !endif
      ...
      if ( j == 0 .or. k == 0 ) then
         ...
         write(6,'(a,1f12.4)') 'epsx: ',epsx(i,j,k)/epslow
         call mexit(6,1)
      end if
      if ( i == 0 .or. k == 0 ) then
         continue
         ...
         write(6,'(a,1f12.4)') 'epsy:',epsy(i,j,k)/epslow
         call mexit(6,1)
      end if
      if ( i == 0 .or. j == 0 ) then
         continue
         ...
         write(6,'(a,1f12.4)') 'epsz:',epsz(i,j,k)/epslow
         call mexit(6,1)
      end if
   end do; end do; end do
...
!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
!+ Assign Spline-smoothed eps values for offset z - R.Q.
!+ Fitted range : 2.5 ~ 20.0
function epsspline( oz )
    ...
    if ( oz < 2.5d0 ) then
        epsspline = 1.0d0
    else if ( oz >= 2.5d0 .and. oz < 5.0d0 ) then
        ! CubicSpline
        epsspline = 0.034417256384*oz**3 - 0.4118570005296*oz**2 + 1.413961445448*oz - 0.49856699131
    else if ( oz >= 5.0d0 .and. oz < 7.5d0 ) then
        ! 5.0~20.0 UnivariateSpline
        epsspline = 0.0116292996139926*oz**2 - 0.239607327187515*oz + 1.48427641627793
    else if ( oz >= 7.5d0 .and. oz < 12.5d0 ) then
        epsspline = 0.0039424581267718*oz**2 - 0.124304704879204*oz + 1.05189158262176
    else if ( oz >= 12.5d0 .and. oz < 20.0d0 ) then
        epsspline = 0.00165991953801297*oz**2 - 0.067241240160233*oz + 0.695244928128194
    else
        epsspline = 0.0125d0
    end if

end function epsspline
!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
!+ Assign PCHIP-interpolated eps values for offset z - R.Q.
!+ Fitted range : 2.5 ~ 20.0, with zero-slope boundary condition
function epspchip( oz )
    ...
    if ( oz < 2.5d0 ) then
        ...
        epspchip = 0.0502857142857143*oz**3 - 0.582857142857143*oz**2 + 1.97142857142857*oz - 1.07142857142857
    else if ( oz >= 5.0d0 .and. oz < 10.0d0 ) then
        epspchip = -0.00120634920634921*oz**3 + 0.0312698412698413*oz**2 - 0.307936507936508*oz + 1.40873015873016
    else if ( oz >= 10.0d0 .and. oz < 15.0d0 ) then
        epspchip = 0.000916959064327486*oz**3 - 0.0312046783625731*oz**2 + 0.304561403508772*oz - 0.592105263157895
    else if ( oz >= 15.0d0 .and. oz < 20.0d0 ) then
        epspchip = 9.47368421052631e-5*oz**3 - 0.00371052631578947*oz**2 + 0.0347368421052631*oz + 0.0440789473684218
    else
        epspchip = 0.0125d0
    end if

end function epspchip
!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
subroutine epsfracx_r( i, j, k, a, b, a1, b1, rh, epsint, depsin, depsout, u )
   ...
   if ( sasopt == 2 .or. sasopt == 3 ) nbndx = nbndx + 1
   fedgex(nbndx) = t - dble(i) ! R. Luo: why the saved fraction is not just aa?
   epsint = (depsout*depsin)/(depsin*(1.0d0-aa) + depsout*aa)

   ! ipb2 surface rendering statements
   !write(str,'(i10)') nbndx+nbndy+nbndz
   !str = adjustl(str)
   !write(58,'(a,3(f20.15),2x)') "H"//str, t*h+gox, j*h+goy, k*h+goz
...
subroutine epsfracy_r( i, j, k, a, b, a1, b1, rh, epsint, depsin, depsout, u )
   ...
   if ( abs(aa) < 1.d-12 ) aa = 0.d0
   if ( sasopt == 2 .or. sasopt == 3 ) nbndy = nbndy + 1
   fedgey(nbndy) = t - dble(j) ! R. Luo: why the saved fraction is not just aa?
   epsint = (depsout*depsin)/(depsin*(1.0d0-aa) + depsout*aa)

   ! ipb2 surface rendering statements
   !write(str,'(i10)') nbndx+nbndy+nbndz
   !str = adjustl(str)
   !write(58,'(a,3(f20.15),2x)') "H"//str, i*h+gox, t*h+goy, k*h+goz
...
subroutine epsfracz_r( i, j, k, a, b, a1, b1, rh, epsint, depsin, depsout, u )
   ...
   if ( abs(aa) < 1.d-12 ) aa = 0.d0
   if ( sasopt == 2 .or. sasopt == 3 ) nbndz = nbndz + 1
   fedgez(nbndz) =  t - dble(k) ! R. Luo: why the saved fraction is not just aa?
   epsint = (depsout*depsin)/(depsin*(1.0d0-aa) + depsout*aa)

   ! ipb2 surface rendering statements
   !write(str,'(i10)') nbndx+nbndy+nbndz
   !str = adjustl(str)
   !write(58,'(a,3(f20.15),2x)') "H"//str, i*h+gox, j*h+goy, t*h+goz
...
!  half_h = HALF*h
   ...
   do ip = 1, nbnd
      ...
      dx(1:3) = g(1:3) - x(1:3); dx2 = dx(1)**2 + dx(2)**2 + dx(3)**2
!     hh = half_h
