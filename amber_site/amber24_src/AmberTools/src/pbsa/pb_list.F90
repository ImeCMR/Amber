   ...
   do i = 1, natom
      ...
      do j = 1, natom
         ...
         do kp = nshrt(i-1) + 1, nshrt(i)
            ...
            if (k == j) then
               eclose = eclose + 1
               tmpex(eclose) = j ! This is unnecessary, as all info has been stored in natex/nshrt. R.Q.
               j_is_excluded = .true.
               exit
            end if
         ...
         ! The following loop is unnecessary. The boolean i_is_excluded
         ! is always false, i.e. j never excludes i (<j) - R.Q.
         !------------- <loop> -----------------
         i_is_excluded = .false.
...
subroutine pb_setgrd( ipb, verbose, pbprint, initial, pbgrid, ifcap, atmfirst, atmlast, xcap, ycap, zcap, cutcap )
   ...
   if ( .not. initial ) then
      ...
      if ( ipb < 4 ) deallocate(saltgrd, stat = alloc_err(6 ) )
      ...
      if ( ipb < 4 ) deallocate( fedgex, stat = alloc_err(14) )
      if ( ipb < 4 ) deallocate( fedgey, stat = alloc_err(15) )
      if ( ipb < 4 ) deallocate( fedgez, stat = alloc_err(16) )
      deallocate( iepsav, stat = alloc_err(17) )
      if ( ipb < 4 ) deallocate(iepsavx, stat = alloc_err(18) )
      if ( ipb < 4 ) deallocate(iepsavy, stat = alloc_err(19) )
      if ( ipb < 4 ) deallocate(iepsavz, stat = alloc_err(20) )
