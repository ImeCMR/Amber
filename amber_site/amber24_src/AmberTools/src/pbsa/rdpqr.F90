...
! TAG AtomNumber AtomName ResidueName ChainID ResidueNumber X Y Z Charge Radius Element
!
...
! Element: The atom element label, optional, which will be ignored.
!
   ...
   do
      ...
      if ( outcome == 0 ) then
         ...
         if ( trim(word) .eq. 'ATOM' .or. trim(word) .eq. 'HETATM' ) then
            ...
            if ( n .eq. 10 ) then
               chainopt = 0      ! barebone version with neither chainid nor element
            else if ( n .eq. 11 ) then
               call get_token(line, 5, word)
               if ( len(trim(word)) == 1 ) then
                  chainopt = 11  ! so we have chainid but no element
               else
                  chainopt = 12  ! so we have no chainid but element
               end if
            else if ( n .eq. 12 ) then
               chainopt = 2      ! full version both both chainid and element
            else
            ...
            if ( chainopt == 0  ) read(line, *) tag, atmnum, igraph(i), resid, resnum, &
                                          acrd(1,i), acrd(2,i), acrd(3,i), acrg(i), radi(i)
            if ( chainopt == 11 ) read(line, *) tag, atmnum, igraph(i), resid, resnum, &
                                          acrd(1,i), acrd(2,i), acrd(3,i), acrg(i), radi(i), element
            if ( chainopt == 12 ) read(line, *) tag, atmnum, igraph(i), resid, chainid, resnum, &
                                          acrd(1,i), acrd(2,i), acrd(3,i), acrg(i), radi(i)
            if ( chainopt == 2  ) read(line, *) tag, atmnum, igraph(i), resid, chainid, resnum, &
                                          acrd(1,i), acrd(2,i), acrd(3,i), acrg(i), radi(i), element
