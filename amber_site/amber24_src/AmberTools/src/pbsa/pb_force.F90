...
#  include "extra.h"
...
#if !defined SANDER
   integer ier
   integer, allocatable :: jgroup(:), irespw(:)
   ...
   call pbtimer_start(PBTIME_PBSETUP)
   if ( (sasopt .le. 2) .and. (ifcap == 0 .or. ifcap == 5) ) then
      call sa_free
...
#ifdef nvCUDA
      ! Mute this for MMPBSA CUDA - R.Q.
      ! Don't mute for 2019 release, so CUDA macro doesn't take effect here. 
      ! Need to add a unique flag to turn on all the options for MMPBSA CUDA, while leaving normal PBSA CUDA use untouched - R.Q.
   ...
   !if (ipb /= 2 .or. sasopt /= 2) then ! Temporary for ipb=2 - R.Q.
   !   call pb_directnocut(natom,proatm,inatm,ipres,ibgwat,ienwat,ibgion,ienion,ntypes,eneopt,idecomp,ifcap, &
   !           iac,ico,nex,iex,cn1,cn2,acg,acrd(1,1),pbfrc,eel,enb)
   !endif
#else
