      ...
      do level = 1, nfocus
         ...
         if ( ifcap /= 1 ) then
            ...
            if ( membraneopt > 0 ) then
               ...
               ! center
                  mzmin = (czbox(1) - mthick*HALF - goz)/h
                  mzmax = (czbox(1) + mthick*HALF - goz)/h
                  mzctr = (czbox(1) - goz)/h ! for z-denpendent eps
               else
               ! Otherwise, use the read in center as the membrane center
                  ...
                  if ( mzmin < zmin .or. mzmax > zmax ) then
                     write(6,'(a)') 'PBSA Warning: in pb_fdfrc()'
                  ...
                  mzmin = (mzmin - goz)/h
                  mzmax = (mzmax - goz)/h
                  mzctr = (mctrdz - goz)/h ! for z-denpendent eps
               end if
            end if
         ...
         if ( ipb >= 1 .and. ipb <=3 ) then
            ...
            call pb_anaiim( npbstep,npbgrid,nstlim,1,natom,npbopt,solvopt,level,nfocus,bcopt,&
               ...
               bv,cphi,xs(xsoffset) )
#else
            write(6,'(a)') "ANA-IIM is not supported in SANDER or LIBPBSA";call mexit(6,1)
#endif /*#if !defined SANDER && !defined LIBPBSA*/
         else if ( (ipb == 7) .or. (ipb == 8) ) then
            if ( ligand ) then
               write(6,'(a)') "ANA-IIM doesn't work with ligandmask."; call mexit(6,1)
            end if
#if !defined SANDER && !defined LIBPBSA
            call pb_nhadrv( npbstep,npbgrid,nstlim,1,natom,npbopt,solvopt,level,nfocus,bcopt,&
               natom,h,savh,gox,goy,goz,savgox,savgoy,savgoz,&
               xm,ym,zm,xmym,xmymzm,savxm,savym,savzm,&
               maxitn,itn,fmiccg,accept,laccept,wsor,lwsor,inorm,norm,&
               pbkappa,pbkb,pbtemp,ivalence,istrng,eps0,epsin,epsout,ionene,&
...
#else
            write(6,'(a)') "IMPROVED-HA is not supported in SANDER or LIBPBSA";call mexit(6,1)
#endif /*#if !defined SANDER && !defined LIBPBSA*/
      ...
      ! dbf is computed by Cai and Ye et al
         ...
         if ( frcopt == 2 ) then
            ...
            call pb_lpbene( pbverbose,pbprint,natom,lastatm,ifcap,&
               npdec,idecomp,m,irespw,ipres,eelrf,insas,phi,chgrd,cphi, ipb ) ! Added ipb option for CUDA - R.Q.
            else
...
subroutine pb_lpbene( verbose,eneout,natom,atmlast,ifcap,npdec,idecomp,m,irespw,ipres, &
                     eel,insas,phi,chgrd,cphi, ipb ) ! Added ipb option for CUDA - R.Q.

#ifdef nvCUDA
   use solvent_accessibility, only : dprob, radi, arccrd, msrf_cu, mtri_cu ! R.Q.
#else
   ...
   if ( prt_srfcrg == 1 ) then
      open (unit=65, file='surf-charge.dat')
      do ip = 1, nbnd
         i = iepsav(1,ip); j = iepsav(2,ip); k = iepsav(3,ip); iatm = iepsav(4,ip)
         write (65, '(3i6,e20.6)' ) i, j, k, pol_charge(ip)
      end do
      close(65)
   else if ( prt_srfcrg == 2 ) then
      allocate (pol_phi(1:atmlast))
      pol_phi = ZERO
   end if
   ...
   ! For CUDA R.Q.
#ifdef nvCUDA
      alen = msrf_cu + mtri_cu ! For passing arccrd memory to CUDA
   if (ipb == 2 .and. alen /= 0) then ! Note alen /=0 check is not a roubust way. Filter for ipb2 and sasopt2 giving alen=0. Improve the setting later - R.Q.
      !write(6, *) 'msrf_cu, mtri_cu', msrf_cu, mtri_cu, alen
      call cuda_lpbene(nbnd, real(iepsav), real(pol_charge), atmlast, real(acrd), &
         real(acrg), real(gox), real(goy), real(goz), real(h), eel_t, real(dprob), &
         real(radi), real(arccrd), alen)
      ...
      do ip = 1, nbnd
         ...
         ! For CUDA, skip pairwise decomp first - R.Q.
#if defined SANDER 
         !-- PB pairwise decomp
   ...
   do ip = 1, nbnd
      ...
      if (idecomp < 3) then
         do jatm = 1, atmlast
            ...
            if ( prt_srfcrg == 2 ) pol_phi(jatm) = pol_phi(jatm) + acg*dinv
   ...
   if ( prt_srfcrg == 2 ) then
      pol_phi(:) = AMBER_ELECTROSTATIC2*pol_phi(:)
      open (unit=65, file='reaction-potential.dat')
      do ip = 1, atmlast
         write (65, '(i6,e20.6)' ) ip, pol_phi(ip)
      end do
      close(65)
   end if
...
subroutine pb_dbfrc_fld2( verbose,eneout,natom,eelrf,f,epsx,epsy,epsz,zv,phi,chgrd,cphi )
   ...
   do ip = 1, nbndx
      ...
      elseif ( iatm > 0 ) then
         x(1:3) = acrd(1:3,iatm)
         rsphere = radi(iatm)
         rn(1:3) = crd(1:3) - x(1:3)
         rn1(1:3) = crd1(1:3) - x(1:3)
      else
         x(1:3) = arccrd(1:3,-iatm)
         rsphere = dprob
         rn(1:3) = x(1:3) - crd(1:3)
         rn1(1:3) = x(1:3) - crd1(1:3)
      end if
