     ...
     &                    eigval_tol,eigvals,eigvecs,hess_weights,
     &                    spectrum,need_eigvecs,ierr,debug_arpack,
...
c
     ...
     &        need_eigvecs,ierr,debug_arpack,return_flag,label
      ...
      Double precision hess_weights(n_dim)
...
c
               status_flag = 0
 17            continue
                  call hessvec ( n, v(1,j), ax, xyz, grad, hess_weights,
     &                           return_flag, status_flag )
